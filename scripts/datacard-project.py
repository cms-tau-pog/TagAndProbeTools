...
parser.add_option("-c", "--decay-channel", dest="channel", default="Comb3L", type="choice", help="Tau discriminator. [Default: Comb3L]", choices=["Comb3L", "Comb3M", "Comb3T", "MVAVL", "MVAL","MVAM","MVAT","MVAVT"])
...
if not options.channel == "new" :
    ...
    os.system("cp {source}/taupog_{channel}.inputs{type}*.root {target}".format(source=source_path, channel=options.channel, type=type, target=target_path))
else:
